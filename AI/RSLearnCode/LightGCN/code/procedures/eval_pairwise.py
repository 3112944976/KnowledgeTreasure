...
#     """
#     Args:
#         X (tuple): 一批包含(预测值,标签)的测试数 X：[(user1_pred_K, user1_ground_items), ...]
...
#     """
#     # 从测试数据中提取出预测评分，并转换为numpy数组：[[user1_pred_k], ...]
#     sorted_items = X[0].numpy()
#     # 从测试数据中提取出真实标签：[[user1_ground_items], ...]
#     ground_truth = X[1]
#     # 获得每个用户对物品的评分情况：[[user1_pred_item1_isTrue 1 also 0, ...], ...]
...
#         "ndcg": np.array(ndcg),
#         "diversity": utils.mean_intra_list_distance(recommendation_lists=sorted_items,item_embeddings=item_embeddings),
...
def test_one_batch(X, item_embeddings, batch_user_bins, batch_user_interaction_history, num_bins=20):
    ...
    for i_k, k in enumerate(world.topks):
        ...
        for user in range(len(sorted_items)):
            ...
            user_ndcg = utils.ndcg_at_k_r(user_ground_truth, user_label, k)
            # 更新累加值
            sum_precision += user_precision
            sum_recall += user_recall
            sum_ndcg += user_ndcg
            count += 1
            # 更新探索与精确度或召回率之间的关系
            exploration_vs_precision[i_k, user_bin] += user_precision
            ...
            exploration_vs_ndcg[i_k, user_bin] = user_ndcg
        # 计算平均值
        precision.append(sum_precision / count)
        recall.append(sum_recall / count)
        ndcg.append(sum_ndcg / count)
    return {
        "recall": np.array(recall),
        "precision": np.array(precision),
        "ndcg": np.array(ndcg),
...
def eval_pairwise(dataset, model, multicore=0):
    ...
    Returns:
        dict: A dictionary containing evaluation metrics.
    """
    batch_size = world.config["test_u_batch_size"]  # --testbatch
    ...
    with torch.no_grad():
        ...
        for batch_users in tqdm(utils.minibatch(users, batch_size=batch_size), desc="Validation", total=total_batch, leave=False):
            ...
            ground_truth_list.append(ground_truth)  # [[all_poc_item], [all_poc_item], ...]
        # 断言总批次数与用户列表的长度相等。
        ...
        # 计算用户根据交互次数分组的列表：[[user_to_bin, ...], 批次元素...]
        ...
        # 将累加的值除以 pre_results 的长度以获取平均值
        num_results = len(pre_results)
        results["recall"] /= num_results
        results["precision"] /= num_results
        results["ndcg"] /= num_results
        results["diversity"] /= num_results
        results["novelty"] /= num_results
        results["exploration_vs_precision"] /= num_results
        results["exploration_vs_recall"] /= num_results
        results["exploration_vs_ndcg"] /= num_results
        # print("exploration_vs_ndcg", results["exploration_vs_ndcg"])
        # print("exploration_vs_ndcg", results["exploration_vs_recall"])
        # print("exploration_vs_ndcg", results["exploration_vs_precision"])
    return results